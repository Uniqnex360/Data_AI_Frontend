...
interface ProjectsTableProps {
  ...
  toggleProjectSelection: (projectId: string, e?: React.MouseEvent) => void;
...
export default function ProjectsTable({
  ...
  return (
    ...
    <th className="px-4 py-3 text-left text-xs font-medium text-slate-600">In Enrichment</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-slate-600">
                Status
              </th>
              <th className="px-4 py-3 text-left text-xs font-medium text-slate-600">
          ...
          <div className="w-16 h-2 bg-slate-200 rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full ${
                (project.completeness_score || 0) >= 90 ? "bg-green-500" :
                (project.completeness_score || 0) >= 50 ? "bg-amber-500" : "bg-red-400"
              }`}
              style={{ width: `${project.completeness_score || 0}%` }}
            />
          </div>
          <span className="text-xs text-slate-600">{project.completeness_score || 0}%</span>
        </div>
      </td>
      ...
      <td className="px-4 py-4">
        {projectEnrichmentCounts[project.id] > 0 ? (
          <span className="px-2 py-0.5 bg-purple-100 text-purple-700 text-xs rounded-full flex items-center gap-1 w-fit">
            ...
            {projectEnrichmentCounts[project.id]} products
          </span>
        ) : (
          <span className="text-slate-400 text-xs">—</span>
        )}
                  ...
                  <td className="px-4 py-4">
                    {getStatusBadge(project.source_status || "Yet to Start")}
                  </td>
