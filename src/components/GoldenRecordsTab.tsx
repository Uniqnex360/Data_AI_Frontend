...
export default function GoldenRecordsTab() {
  ...
  const [selectedRecord, setSelectedRecord] = useState<GoldenRecord | null>(
    null,
  );
  ...
  const publishableCount = records.filter(
    (r) => r.ready_for_publish && !r.published_at,
  ).length;
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    <p className="font-medium text-sm">
                      {record.product_code || (record as any).sku}
                    </p>
                  ...
                  {record.brand && (
                    <p className="text-xs text-slate-600">
                      {record.brand_name || (record as any).brand}
                    </p>
                  )}
                  {record.published_at && (
          ...
          {selectedRecord ? (
            <div className="space-y-4">
              ...
              <div className="p-4 bg-white border border-slate-200 rounded-lg">
                ...
                <div className="space-y-2 max-w-full overflow-x-hidden">
                  {Object.entries(selectedRecord.attributes).map(
                    ([key, value]: [string, any]) => {
                            ...
                            .map(([k, v]) => {
                              const displayInner =
                                typeof v === "object"
                                  ? JSON.stringify(v)
                                  : String(v);
                              return `${k.replace(/_/g, " ")}: ${displayInner}`;
                            })
                            .join(" | ");
                        }
                      ...
                      return (
                        <div
                          key={key}
                          className="flex flex-col sm:flex-row items-start justify-between p-2 bg-slate-50 rounded gap-4 overflow-hidden"
                        >
                          <span className="text-sm font-bold text-slate-500 uppercase tracking-tight flex-shrink-0 sm:w-1/3">
                            {key.replace(/_/g, " ")}
                          </span>

                          <span className="text-sm text-slate-700 flex-1 break-words overflow-hidden min-w-0 text-left sm:text-right font-mono bg-white/50 px-2 py-1 rounded">
                            {formatValue(value)}
                          </span>
                        </div>
                      );
              ...
              <div className="p-4 bg-white border border-slate-200 rounded-lg">
                ...
                {selectedRecord.enrichment &&
                  typeof selectedRecord.enrichment === "object" && (
                      ...
                      {(selectedRecord.enrichment as any).bullets && (
                        <div>
                          ...
                          <ul className="space-y-1">
                            {(selectedRecord.enrichment as any).bullets.map(
            ...
            <div className="text-center py-16 bg-slate-50 rounded-lg border border-slate-200">
              ...
              <p className="text-slate-600">
                Select a golden record to view details
              </p>
            </div>
          )}
        </div>
